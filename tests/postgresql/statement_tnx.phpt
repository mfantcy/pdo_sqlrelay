--TEST--
PDO SQLRELAY POSTGRESQL statement tnx
--SKIPIF--
...
$pdo->query("CREATE TABLE test (a int)");
$pdo->query("INSERT INTO test VALUES (165)");
...
function run($pdo, $data)
{
    ...
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM test WHERE a IN ({$bind})");
