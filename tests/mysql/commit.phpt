...
try {
	...
	$db->exec(sprintf('CREATE TABLE test_commit(id INT) ENGINE=%s', PDOSqlrelayMysqlTestConfig::getStorageEngin()));
	...
	$db = PDOSqlrelayMysqlTestConfig::PDOFactory();
	PDOSqlrelayMysqlTestConfig::createTestTable($db, 'MyISAM');
...
PDOSqlrelayMysqlTestConfig::dropTestTable($db);
?>
--EXPECT--
done!