...
function blob_from_stream($offset, $db, $file, $blob) {
		...
		$sql = sprintf('CREATE TABLE test(id INT, label BLOB) ENGINE=%s', PDOSqlrelayMysqlTestConfig::getStorageEngin());
		$db->exec($sql);
...
$blob = 'I am a mighty BLOB!' . chr(1) . "I am a binary thingie!";
$tmp = sys_get_temp_dir();
$file = $tmp . DIRECTORY_SEPARATOR . 'pdoblob.tst';
...
$db = PDOSqlrelayMysqlTestConfig::PDOFactory();
$db->exec('DROP TABLE IF EXISTS test');
@unlink(sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'pdoblob.tst');
?>
--EXPECTF--
Emulated PS...
Native PS...
