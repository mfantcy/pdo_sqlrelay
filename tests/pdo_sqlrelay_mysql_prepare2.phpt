...
try {
	...
	prepex(6, $db, sprintf('CREATE TABLE test(id INT, label CHAR(255)) ENGINE=%s', PDOSqlrelayMysqlTestConfig::getStorageEngin()));
	...
	$sql = sprintf("SELECT id, label FROM test WHERE (label LIKE '%s') AND (id = :placeholder)", '%ro%');
	$stmt = prepex(29, $db, $sql,	array('placeholder' => -1));
	if (count(($tmp = $stmt->fetchAll(PDO::FETCH_ASSOC))) != 0)
			printf("[030] Expecting zero rows, got %d rows\n", $tmp);

	$sql = sprintf("SELECT id, label FROM test WHERE  (id = :placeholder) OR (label LIKE '%s')", '%go%');
	...
	prepex(34, $db, sprintf('CREATE TABLE test(id INT, label CHAR(255)) ENGINE=%s', PDOSqlrelayMysqlTestConfig::getStorageEngin()));
	...
	*/
	...
	$sql = sprintf("SELECT id, label FROM test WHERE (label LIKE '%s') AND (id = ?)", '%ro%');
	$stmt = prepex(60, $db, $sql,	array(-1));
	if (count(($tmp = $stmt->fetchAll(PDO::FETCH_ASSOC))) != 0)
			printf("[061] Expecting zero rows, got %d rows\n", $tmp);

	$sql = sprintf("SELECT id, label FROM test WHERE  (id = ?) OR (label LIKE '%s')", '%ro%');
